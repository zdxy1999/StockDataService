...
class InvestPlan:
    """投资计划"""
    plan_name: str
    strategy_path: str   # 相对于 yaml 文件的绝对化后路径
    first_plan_day: str  # YYYYMMDD
    interval: int        # 调仓间隔（交易日数量，0=每日调仓）
    top: int             # 取选股结果前 N 名
    yaml_path: str       # 计划 yaml 文件的绝对路径（用于定位策略文件）
...
class PlanParser:
    ...
    def parse_from_file(yaml_file: str) -> InvestPlan:
        ...
        # strategy 路径相对于 yaml 文件所在目录解析
        strategy_abs = os.path.abspath(
            os.path.join(os.path.dirname(yaml_abs), strategy_raw)
        )
        if not os.path.exists(strategy_abs):
            raise FileNotFoundError(f"策略文件不存在：{strategy_abs}")
