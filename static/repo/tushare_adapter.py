...
class DfWithColumnDesc:
    ...
    def __init__(self, df: pd.DataFrame, desc, column_desc: str):
        self.df = df
        self.desc = desc
...
def get_ipo_stocks_of_a_day(date_str: str = datetime.now().strftime("%Y%m%d")) -> DfWithColumnDesc:
    ...
    ballot	float	Y	中签率
    """
    return DfWithColumnDesc(df, desc, column_desc)
...
def get_ind_money_flow(date_str: str = datetime.now().strftime("%Y%m%d")) -> DfWithColumnDesc:
    """
    获取某一日的行业资金流向
    """
    date_str = adapt_date_str(date_str)
    pro = get_pro()
    df = pro.moneyflow_ind_ths(trade_date=date_str)
    desc = "行业资金流向"
    column_desc = """
    trade_date	str	Y	交易日期
    ts_code	str	Y	板块代码
...
def get_mkt_dc_money_flow(date_str: str = datetime.now().strftime("%Y%m%d")) -> DfWithColumnDesc:
    """
    获取某一日 大盘资金流向
    """
    date_str = adapt_date_str(date_str)
...
def get_cpi_in_this_year(year_str: str = datetime.now().strftime("%Y")) -> DfWithColumnDesc:
    ...
    return DfWithColumnDesc(df, desc)
...
if __name__ == '__main__':
    ...
    # pro = ts.pro_api()
    # df = pro.moneyflow_hsgt(start_date='20250125', end_date='20250808')
    # print(df)
